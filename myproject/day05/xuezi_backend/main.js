...
let pool = mysql.createPool({
    ...
    port:               '3306',     // 端口的''可加可不加，即写成3306也可
    user:               'y4jo401ymz',
    password:           'l014i2lwkxhll2wkhm4k2ly0x331mik0k102421w',
    database:           'app_xzbreezeli',
 ...
 * 1.5首页数据
 */
server.get('/index', function(req, res){    // 并发，线程？异步，同步
    let output = {
        carouselItems: [],
        recommendedItems: [],
        topSaleItems: [],
    ...
    let loadedCount = 0     // 记录现在有几类数据加载完成，if ==4,则说明四类数据全部加载完成，可以返回响应消息
    // 执行数据库查询1：轮播广告条目
    let sql1 = 'SELECT * FROM xz_index_carousel'
    pool.query(sql1, function(err, result){
        if(err){
            throw err;
        }
        output.carouselItems = result
        loadedCount++                   // 需要判断是否向客户端返回数据时候菜需要判断.length>0
        if(loadedCount===4){
            res.json(output)
        }
    })
    ...
    // 执行数据库查询2：首页推荐条目
    let sql2 = 'SELECT pid, title, details, pic, price, href FROM xz_index_product ORDER BY seq_recommended LIMIT 6'
    pool.query(sql2, function(err, result){
        ...
        loadedCount++                   // 需要判断是否向客户端返回数据时候菜需要判断.length>0
        if(loadedCount===4){              // 判断是否所有（四个种类）数据都加载完成
            res.json(output)
        }
    })

    // 执行数据库查询3：热销单品条目
    let sql3 = 'SELECT pid, title, details, pic, price, href FROM xz_index_product ORDER BY seq_top_sale LIMIT 6'
    ...
    // 执行数据库查询4：最新上架条目
    let sql4 = 'SELECT pid, title, details, pic, price, href FROM xz_index_product ORDER BY seq_new_arrival LIMIT 6'
    pool.query(sql4, function(err, result){
        if(err){
            throw err;
        }
        output.newArrivalItems = result
        loadedCount++                   // 需要判断是否向客户端返回数据时候菜需要判断.length>0
        if(loadedCount===4){              // 判断是否所有（四个种类）数据都加载完成
            res.json(output)
        }
